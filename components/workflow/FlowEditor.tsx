...
function FlowEditorInner({ workflowId, initialData, dynamicOptions }: FlowEditorProps) {
    ...
    return (
        <div className="flex w-full h-full bg-background border rounded-lg overflow-hidden relative">
            ...
            <div className="flex-1 relative h-full flex" ref={reactFlowWrapper}>
                <ReactFlow
                    nodes={nodes}
                    edges={edges}
                ...
                <div className={`
                    absolute top-0 right-0 h-full w-full transform transition-all duration-300 ease-in-out z-20 shadow-xl md:shadow-none
                    ${selectedNode ? 'translate-x-0 md:w-80 md:relative md:border-l bg-background' : 'translate-x-full md:w-0 md:opacity-0 md:hidden'}
                `}>
                    {selectedNode && (
                                    ...
                                    return (
                                        <div key={inputSchema.name} className="space-y-1">
                                            ...
                                            {inputSchema.name === 'applicationType' ? (
                                                ...
                                                <textarea
                                                    className="flex min-h-[80px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm font-mono shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
