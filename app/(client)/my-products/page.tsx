                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {applications.length === 0 ? (
                        ...
                        applications.map(app => {
                            // For approved applications, look up the real account record
                            const realAccount = app.status === 'approved'
                                ? realAccounts.find(a => a.user_id === app.user_id)
                                : null;
                                ...
                                <Card key={app.id} className="border shadow-sm flex flex-col">
                                    <CardHeader className="pb-2 flex-1">
                                        ...
                                        <CardTitle className="text-2xl font-mono">
                                            {realAccount
                                                ? `₦${realAccount.balance.toLocaleString('en-NG', { minimumFractionDigits: 2 })}`
                                    ...
                                    <CardContent className="mt-4 pt-4 border-t bg-muted/10">
                                        {realAccount && (
                                            <div className="flex items-center justify-between text-xs text-muted-foreground mb-2">
                                                ...
                                                <span className="font-medium text-foreground">{realAccount.account_type}</span>
                                            </div>
                                        )}
