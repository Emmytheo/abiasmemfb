        ...
        <div className="p-4 sm:p-8 max-w-5xl mx-auto flex flex-col gap-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                    <Link href="/workflows/runs" className="self-start p-2 hover:bg-muted rounded-full transition-colors flex-shrink-0">
                        <ArrowLeft size={20} />
                    </Link>
                    <div>
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                            <h1 className="text-xl sm:text-2xl font-bold tracking-tight">Execution Details</h1>
                            <Badge variant="outline" className="font-mono text-xs opacity-70 border-dashed w-fit">
                                {id}
                            </Badge>
                        </div>
                ...
                <div className="self-end sm:self-auto flex-shrink-0">
                    <DeleteRunButton id={id} redirectUrl="/workflows/runs" />
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        {phases.length === 0 ? (
                            ...
                            phases.map((phase: any, index: number) => (
                                <div key={`${phase.nodeId}-${index}`} className="border rounded-lg bg-card overflow-hidden shadow-sm">
                                    <div className="p-4 flex flex-col sm:flex-row sm:items-center justify-between bg-muted/20 border-b gap-3 sm:gap-0">
                                    ...
                                    {/* Phase Body */}
                                    <div className="p-4 space-y-4 text-sm max-w-full overflow-hidden">
                                        {/* Outputs */}
                                        {Object.keys(phase.outputs || {}).length > 0 && (
                                            <div>
                                                <div className="text-xs font-semibold text-muted-foreground uppercase mb-2">Outputs</div>
                                                <pre className="bg-muted/50 p-3 rounded-md overflow-x-auto text-[10px] sm:text-[11px] font-mono border whitespace-pre-wrap sm:whitespace-pre">
                                        ...
                                        {phase.logs && phase.logs.length > 0 && (
                                            <div>
                                                ...
                                                <div className="bg-black text-green-400 p-3 rounded-md overflow-x-auto text-[10px] sm:text-[11px] font-mono space-y-1">
                                                    {phase.logs.map((log: any, i: number) => (
                                                        <div key={i} className={`flex gap-3 ${log.level === 'error' ? 'text-red-400' : log.level === 'warn' ? 'text-amber-400' : ''}`}>
                                                            <span className="opacity-50 select-none whitespace-nowrap">[{format(new Date(log.timestamp), 'HH:mm:ss')}]</span>
                                                            <span className="opacity-50 w-12 select-none flex-shrink-0">[{log.level.toUpperCase()}]</span>
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="border rounded-lg bg-card p-4 shadow-sm w-full overflow-hidden">
                        <h3 className="font-semibold text-sm mb-4">Input Data</h3>
                        <pre className="bg-muted p-3 rounded-md overflow-x-auto text-[10px] sm:text-[11px] font-mono border">
                    ...
                    <div className="border rounded-lg bg-card p-4 shadow-sm">
                        ...
                        <ul className="text-sm space-y-2 text-muted-foreground">
                            ...
                            {run.idempotencyKey && (
                                <li className="flex justify-between items-center gap-2 mt-2 pt-2 border-t">
                                    <span className="whitespace-nowrap">De-dup Key</span>
                                    <span className="font-mono text-[10px] truncate">{String(run.idempotencyKey).slice(0, 16)}...</span>
                                </li>
                            )}
                        </ul>
                    </div>
