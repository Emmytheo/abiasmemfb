...
export default function EPinsPage() {
    ...
    useEffect(() => {
        dashboardApi.getTransactionsByCategory("e-Pins").then((txs) => {
            setData(txs);
            setLoading(false);
        });
    }, []);

    if (loading) return <div className="p-8">Loading e-pins...</div>;

    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <GenericDataTable
                title="e-Pins Purchases"
                description="Monitor all customer e-Pin purchases."
                data={data}
