    ...
    useEffect(() => {
        async function loadData() {
            try {
                ...
                // Removing filter so we see all applications
                setData(enriched);
            } finally {
                setLoading(false);
            }
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <GenericDataTable
                ...
                description="Manage and review customer product applications."
                data={data}
                ...
                columns={[
                    {
                        header: "Reference",
                        accessorKey: "id",
                        cell: (item) => <span className="font-mono text-xs">{typeof item.id === 'string' && item.id.includes('_') ? item.id.split('_')[1]?.toUpperCase() : String(item.id).substring(0, 8).toUpperCase()}</span>
                    },
                    {
                        header: "Product",
                    ...
                    <div key={item.id} className="border rounded-lg p-4 bg-background shadow-sm hover:shadow-md transition-shadow relative">
                        <div className="flex justify-between items-start mb-4 gap-2">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground font-mono mt-1">Ref: {typeof item.id === 'string' && item.id.includes('_') ? item.id.split('_')[1]?.toUpperCase() : String(item.id).substring(0, 8).toUpperCase()}</p>
                            </div>
